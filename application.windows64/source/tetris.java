...
public int modulo( int num, int divisor ){
  if( num >= divisor ){ 
    while( num >= divisor ){ 
      num -= divisor; 
    } 
...
class Block extends Position {
  ...
  public Block( int theSize, Position pos ){
    ...
    DNA = PApplet.parseInt(random(0,7));
    touched = false;
    x = pos.x;
    y = pos.y;
  ...
  public void smoothGrid( int xG, int yG ){
    if( touched == false ){
      x = PApplet.parseInt( lerp( x, (x/xG) * xG, .5f ) );
      y = PApplet.parseInt( lerp( y, (y/yG) * yG, .5f ) );
    }
  }
...
public void keyPressed(){
  ...
  if( keyPressed && keyCode == RIGHT ){
    ...
    b = newBlock;
    b.moveto(200, 200);
    int dna = PApplet.parseInt(random(0,7));
...
public boolean clearFullRow( ArrayList<Square> grid, int row ){
...
public boolean isBlockInGrid( Block block, ArrayList<Square> squares ){
  for( Square square : b.pieces ){
...
public boolean isBlockOnGrid( Block block, int xLeft, int xRight, int yTop, int yBot ){
  for( Square square : b.pieces ){
...
public void keepOnGrid( Block blk, int xLeft, int xRight, int yTop, int yBot, int blockLength ){
